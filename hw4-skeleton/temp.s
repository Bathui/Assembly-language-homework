...
misaligned_load:
    ...
    srli t3 t3 12
    li t4 0
    beq t3 t4 else # jump to load half word
    
    lw t2 0(t0)
    # li a7, 1          # Load the ecall number for print integer (1) into a7
    # mv a0, t2         # Move the value in t2 to a0 (the argument for ecall)
    # ecall             # Make the ecall to print the value in a0
    ...
    lb t3 3(t1)   # read from the first element 
    lb t4 2(t1)   # read from the second element
    lb t5 1(t1)   # read from the third element
    lb t6 0(t1)   # read from the fourth element
